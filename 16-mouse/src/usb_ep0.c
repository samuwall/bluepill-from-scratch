...
usb_standard_device_get_status(usb_device *dev, struct usb_setup_data *req, 
...
static enum usb_req_result
usb_standard_device_set_address(usb_device *dev, struct usb_setup_data *req, 
                                uint8_t **buf, uint16_t *len) {
    (void)dev;
    (void)req;
    (void)buf;
...
static enum usb_req_result
usb_standard_device_get_descriptor(usb_device *dev, struct usb_setup_data *req, 
                                   uint8_t **buf, uint16_t *len) {
...
static enum usb_req_result
usb_standard_device_set_configuration(usb_device *dev, struct usb_setup_data *req, 
                                      uint8_t **buf, uint16_t *len) {
...
static enum usb_req_result
usb_standard_req_handler(usb_device *dev, struct usb_setup_data *req, 
                         uint8_t **buf, uint16_t *len) {
    ...
    case USB_REQ_TYPE_DEVICE:
        ...
        switch (req->bRequest) {
            case USB_REQ_GET_STATUS:
                return usb_standard_device_get_status(dev, req, buf, len);
            ...
            case USB_REQ_SET_ADDRESS:
                return usb_standard_device_set_address(dev, req, buf, len);

            case USB_REQ_GET_DESCRIPTOR:
                return usb_standard_device_get_descriptor(dev, req, buf, len);
            ...
            case USB_REQ_SET_CONFIGURATION:
                return usb_standard_device_set_configuration(dev, req, buf, len);
...
/* section 8.5.3 
 */
static void stall_transaction(usb_device *dev) {
...
usb_ep0_handle_request(usb_device *dev, struct usb_setup_data *req) {
    ...
    result = usb_standard_req_handler(dev, req, &(dev->ep0.xfer_buf),
                                    &(dev->ep0.xfer_len));
...
void _usb_ep0_setup(usb_device *dev, uint8_t ep) {
...
void _usb_ep0_in(usb_device *dev, uint8_t ep) {
...
void _usb_ep0_out(usb_device *dev, uint8_t ep) {
 ...
 *          
 *       
 * note 2 :  usb_standard_set_address()  ...  usb_ep0_in() 
 * 
 *          the reason why `usb_standard_set_address()` doesn't actually set the 
