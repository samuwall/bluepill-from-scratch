/********************************************************************
 ** file         : irq.c
 ** description  : USART IRQ : demonstrate NVIC / external interrupts
 **
 **
 ********************************************************************/
...
static void usart_setup(void) {
    ...
    USART2->CR1 = (USART2->CR1 & ~(USART_CR1_MODE_Msk)) | (USART_CR1_MODE_TX_RX << USART_CR1_MODE_Shft);                /* TX mode */
    ...
    USART2->CR1 |= USART_CR1_RXNEIE_;

    /* enable IRQ */
    NVIC->ISER[NVIC_USART2_IRQ / 32] = (1 << (NVIC_USART2_IRQ % 32));
...
void usart2_isr(void) {
    ...
    /* check if called because RXNE=1 */
    if ( (USART2->SR & USART_SR_RXNE_) != 0) {
        
        /* indicate data received */
        blink();

        /* read data from DR */
        data = USART2->DR & USART_DR_DR_Msk;

        /* send data back */
        usart_send_blocking(data);
    }
